...
def main():
    ...
    parser.add_argument('--data_dir', default='data_reformat', help='Root dir for data')
    ...
    parser.add_argument('--max_samples', type=int, default=None,help = 'If set, only load this many passages (for quick testing)')
...
def train(opt, config, data_df, fold=0):
    ...
    #train_dataset = WikipediaDataset(x_train, tokenizer, max_length=80, split='trainval', transforms=clip_transform, include_images=not config['image-model']['disabled'])
