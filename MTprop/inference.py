    ...
    if opt.linear_assignment:
        ...
        if opt.enable_cached_scores and os.path.exists(score_file):
            ...
            if opt.set == 'test':
                ...
                # only test the first 10 photos for now
            ...
            #result_indexes = exhaustive_knn_search(query_feats, caption_feats, opt.set, topk=opt.k, gpu=False, cache_scores=opt.enable_cached_scores)
            result_indexes = exhaustive_knn_search(query_feats, caption_feats, opt.set, topk=opt.k, gpu=False,
                                                   cache_scores=False)
# GPU 要变
    ...
    if opt.submission_file:
        ...
        final_df = final_df[["id", "combined_text"]].rename(columns={"combined_text":"caption_title_and_reference_description"})
